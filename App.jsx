...
export default function App() {
  ...
  const [isLoadingStories, setIsLoadingStories] = useState(true); // Status Loading
  ...
  const handleAuth = async (e) => {
    ...
    try {
        ...
        if (error.message === "kode-salah") msg = "Kode Pendaftaran SALAH! Hubungi admin.";
        ...
        {view === 'home' && (
          <div className="animate-fade-in">
            ...
            {isLoadingStories ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">{stories.map(story => (<div key={story.id} onClick={() => { setCurrentStory(story); setActiveChapterIndex(null); setView('read'); }} className="group cursor-pointer flex flex-col gap-2 relative"><div className="aspect-[2/3] w-full rounded-lg overflow-hidden shadow-md group-hover:shadow-xl transition relative bg-gray-200"><img src={story.coverUrl} alt={story.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" onError={(e) => {e.target.src = 'https://placehold.co/400x600/e2e8f0/1e293b?text=No+Cover'}} /><div className="absolute top-2 right-2 bg-black/60 text-white text-[10px] px-2 py-1 rounded-full backdrop-blur-sm">{story.chapters.length} Bab</div></div><div><h3 className="font-bold text-gray-800 leading-tight group-hover:text-orange-600">{story.title}</h3><p className="text-xs text-gray-500 mt-1 flex items-center gap-1"><User size={10} /> {story.authorName || 'Anonim'}</p></div></div>))}</div>
            )}
          </div>
        )}
